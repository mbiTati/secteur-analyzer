...
const API_CONFIG = {
    ...
    dvfCerema: 'https://apidf-preprod.cerema.fr/dvf_opendata/mutations',
    // Proxies CORS pour MeilleursAgents (fallback chain)
    corsProxies: [
...
async function getDVFTransactions(codeInsee) {
    // Essayer l'API cquest (plus complète)
    try {
        const response = await fetch(
            `${API_CONFIG.dvf}?code_commune=${codeInsee}`,
            { timeout: 10000 }
        );
        ...
        if (response.ok) {
            ...
            if (data.resultats && data.resultats.length > 0) {
                return data.resultats.map(normalizeTransactionCquest);
            }
        }
    } catch (e) {
        console.warn('API cquest indisponible, tentative alternative...');
    }
    
    // Fallback: API Cerema
    try {
        const response = await fetch(
            `${API_CONFIG.dvfCerema}?code_insee=${codeInsee}&ordering=-date_mutation&page_size=100`
        );
        
        if (response.ok) {
            const data = await response.json();
            if (data.results && data.results.length > 0) {
                return data.results.map(normalizeTransactionCerema);
            }
        }
    } catch (e) {
        console.warn('API Cerema indisponible');
    }
    
    // Si aucune API ne fonctionne, retourner un tableau vide
    console.warn('Aucune donnée DVF disponible');
    return [];
}
...
function normalizeTransactionCquest(t) {
    return {
        ...
        surface: t.surface_reelle_bati || t.surface_terrain || 0,
        prix: t.valeur_fonciere || 0,
        prixM2: t.surface_reelle_bati > 0 ? Math.round(t.valeur_fonciere / t.surface_reelle_bati) : 0,
        pieces: t.nombre_pieces_principales || 0
    };
}
...
function normalizeTransactionCerema(t) {
    return {
        ...
        surface: t.sbati || t.sterr || 0,
        prix: t.valeur_fonciere || 0,
        prixM2: t.sbati > 0 ? Math.round(t.valeur_fonciere / t.sbati) : 0,
        pieces: t.nbpprinc || 0
    };
}
...
function displayTransactions(transactions) {
    ...
    document.getElementById('transactionsBody').innerHTML = html || '<tr><td colspan="6" style="text-align: center; color: var(--text-muted);">Aucune transaction disponible</td></tr>';
}
